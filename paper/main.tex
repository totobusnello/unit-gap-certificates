...
\textbf{Both sides of that gap are classical.} The AIG basis with free inversions is $U_2$, where
Schnorr's theorem gives the circuit complexity of $\oplus_n$ as exactly $3n-3$, and Lee's rank
technique gives the De~Morgan formula size of $\oplus_n$ as exactly $2^{\ell}(2^{\ell}+3r)$ leaves
for $n = 2^{\ell}+r$ --- 6 and 9 gates respectively at $n = 3$ (a formula with $L$ leaves has
$L-1$ gates). \textbf{So $\gapop(\oplus_3) = 3$
follows from two published results and the basis identification above, with no computation of
ours in the chain}; the certificates,
the dynamic programs and the Lean development replicate that value mechanically rather than
establish it. (A weaker route needs even less: Khrapchenko forces $\tree(\oplus_3) \ge 8$, which
with the six-gate circuit already gives $\gapop \ge 2 > 1$.) The error is locatable in the
...
\begin{enumerate}
  ...
  Krinkin's shared-gate term is 3. \emph{The counterexample is not new mathematics}: both sides of
  the gap were determined decades ago --- Schnorr for the circuit, Lee for the formula --- so the
  contradiction with Theorem~2 was already implied by the published literature. What we add is to
...
\begin{itemize}
  ...
  $L - 1$ gates --- the same conversion already used for Khrapchenko above. Lee~\cite{lee2007} then
  determines the De~Morgan formula size of parity for every $n$: writing $n = 2^{\ell} + r$ with
  $0 \le r < 2^{\ell}$, the minimum number of leaves is exactly $2^{\ell}(2^{\ell} + 3r)$. For
  $n = 3$ this is 10 leaves, hence 9 two-input gates --- the value the dynamic program and the Lean
  development compute independently. So Theorem~2 is contradicted by Schnorr and Lee alone. Lee's expression also
  explains why the Khrapchenko route stops at $\ge 2$: equality with $n^2$ holds exactly when
  $r = 0$, and Lee notes that no bound above $n^2$ is reachable by Khrapchenko's method at all.
  The same two results predict $\gapop(\oplus_4) = 15 - 9 = 6$, which is the maximum gap found in
  the $n = 4$ census of \S\ref{sec:census} --- an external check on the census, not merely on the
  counterexample.
\end{itemize}
...
$\treex$ maximum 7. The gap census (\texttt{certs}: \texttt{npn4\_xag\_gap.csv}): $n = 3$ ---
...
Fully citational: Schnorr~\cite{schnorr1976} for $\opt = 6$ and Lee~\cite{lee2007} for
$\tree = 9$; the only step supplied here is the basis identification of \S\ref{sec:prelim} \\
\addlinespace
...
\begin{itemize}
  ...
  \item \textbf{Both sizes of parity are classical.} Schnorr~\cite{schnorr1976} determined the
  $U_2$ circuit complexity of $\oplus_n$ exactly, $3n-3$, and Lee~\cite{lee2007} determined its
  De~Morgan formula size exactly. Since the AIG basis with free inversions is
  $U_2$ (\S\ref{sec:prelim}), the two together fix $\opt(\oplus_3) = 6$ and $\tree(\oplus_3) = 9$
  --- hence the gap 3 --- without any computation, and they agree with the values reported here at
  $n = 4$ as well ($\opt = 9$, $\tree = 15$, gap 6, the census maximum). We claim \textbf{no novelty
  for either size}; they are used as an external check and to make the refutation citational. What
...
deduplicated) and the 494/494 re-verification log; and the Lean sources. The $n = 5$ search seeds
are deterministic per worker; the bench is a 2-vCPU VPS.
\end{sloppypar}
