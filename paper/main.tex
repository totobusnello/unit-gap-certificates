...
$\tree(\oplus_3) = 9$, which is not in the refereed literature; that side is certified here by
two independent dynamic programs and the Lean development (the circuit side, $\opt = 6$, by the
DRAT chain), and agrees with an exhaustive enumeration published informally in 2011. At $n = 4$ the exact gap is classical again: Tarui
...
\begin{itemize}
  ...
  and reading them together requires only the basis identification above. Both have been in print
  since 1976, so the facts that contradict Theorem~2 were already on the record when it was
  stated; what was missing was a reason to subtract one from the other.
