"""Census of opt_XAG over the 222 NPN classes of n=4 (EXP-XAG-N4, gates X1+X2 PASSED).
Ascending search k=0,1,... per class; the SAT model is checked by simulation.
Output: npn4_xag.csv + progress on stdout.
"""
import csv
import sys
import os
...
with open(out_path, mode, newline="") as f:
    ...
    for i, r in enumerate(rows):
        ...
        t1 = time.time()
        # opt_XAG <= opt_AIG always — use as kmax (a sound prune, not a heuristic)
        kmax = int(r["opt_aig"])
        opt = opt_via_sat(4, tt, kmax=kmax, verify=True)
        assert opt is not None, f"{r['npn_rep_hex']}: no opt up to kmax={kmax} — would violate opt_XAG<=opt_AIG"
        dt = time.time() - t1
